...
if($tk>0)
{
    ...
    <title> Complaints Restored</title>
    ...
    { 
        ...
        <tr>
        <td colspan="6" align="left"  style=" border-top:none !important;border-bottom:none !important;"><B><?php echo " Complaints Restored Todays";?></B></td>
        <td colspan="7" align="right"  style=" border-top:none !important;"><B>Print Date : <?php echo $print_date; ?> Time : <?php echo $print_dt[1]; ?></B></td>
        </tr>
        </thead> 
        ...
        $sql1.=" and substr(f.close_date,1,10)=CURRENT_DATE ";
...
//        $sth->bindParam(':from_date', $from_date);
        ...
        foreach ($row1 as $key => $value1) 
        {
            ...
            $close_dt=british_to_ansi(substr($close_date,0,10));
            $close_tm=substr($close_date,11,5);
			...
			$sqlR="select user_nm,user_id from user_mas WHERE 1=1 ";
			$sqlR.=" and uid=:close_by ";
			$sth_search = $conn->prepare($sqlR);
			$sth_search->bindParam(':close_by', $close_by);
			$sth_search->execute();
			$ss_search=$sth_search->setFetchMode(PDO::FETCH_ASSOC);
			$row_search = $sth_search->fetch();
//			$close_by=$row_search['user_nm'];
			$close_by=$row_search['user_id'];
