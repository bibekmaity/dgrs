...
class _image {
    ...
    // UPLOAD FUNCTION
  //  public $uploadTo        = './uploads/';    // folder to upload to (relative to calling document)
    
    // RESIZE FUNCTION
    public $source_file     = '';            // source file to resize
    public $newWidth         = '200';            // new image width - e.g. '200' = 200 pixels
    public $newHeight         = '200';            // new image height - e.g. '200' = 200 pixels
    ...
    public function upload($ar) {
        ...
        if(copy($img_tmp_name, $target_path)) {
    ...
    public function resize() {
        // ERROR CAPTURE
        if($this->newWidth=='') {     $this->doDie($this->errors['no-width']); echo "no-width";exit; }
        if($this->newHeight=='') {     $this->doDie($this->errors['no-height']); echo "no-height";exit; }
        $sImage = (($this->source_file!='')&&(file_exists($this->source_file))) ? $this->source_file : $this->doDie($this->errors['no-image']);
...
function upload_img($img_file,$width,$height){
		/*--------------file upload-----------------------*/
	...
	$res = $myImage->upload($img_file);
	if($res) {
		// RESIZE
		//$myImage->padColour = '#222222';
		$myImage->newWidth = $width;
		$myImage->newHeight = $height;
		$i = $myImage->resize();
		echo "<br>$i";
	}
	} 
...
function upload_img1($img_file,$width,$height){
		/*--------------file upload-----------------------*/
